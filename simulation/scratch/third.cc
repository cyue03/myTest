...
//#undef PGO_TRAINING
#define PGO_TRAINING false
#define PATH_TO_PGO_CONFIG "/home/cyue/Desktop/workspace/High-Precision-Congestion-Control/simulation/mix/config_test.txt"
...
int main(int argc, char *argv[])
{
	...
	//仅供调试使用
	topof.open("../mix/fat.txt");
	flowf.open("../mix/flow.txt");
	tracef.open("../mix/trace.txt");
	...
	FILE *fct_output = fopen("../mix/fct.txt", "w");
	//
	// install RDMA driver
	//
	for (uint32_t i = 0; i < node_num; i++){
	...
	FILE *trace_output = fopen("../mix/mix_test.tr", "w");
	if (enable_trace)
	...
	FILE* qlen_output = fopen("../mix/qlen.txt", "w");
	Simulator::Schedule(NanoSeconds(qlen_mon_start), &monitor_buffer, qlen_output, &n);

	FILE *cnp_output = fopen("../mix/cnp.txt", "w");
	fclose(cnp_output);
	//
