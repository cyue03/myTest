...
public:
	...
	std::unordered_map<std::string,int > m_flowTable; // map from dip sip (u32) to possible maxhop
	...
	Ptr<Packet> GenerateMyCnp(CustomHeader &ch);
	uint32_t GetccMode();
    ...
    *****************************/
   	double tfd_ratio;
	double tfd_rateDecreaseInterval;
	double tfd_alpha_resume_interval;
	double tfd_rpgTimeReset;

	void UpdateAlphaTfd(Ptr<RdmaQueuePair> q);
	void ScheduleUpdateAlphaTfd(Ptr<RdmaQueuePair> q);

	void cnp_received_tfd(Ptr<RdmaQueuePair> q);

	void CheckRateDecreaseTfd(Ptr<RdmaQueuePair> q);
	void CheckRateDecrease_Tfd(Ptr<RdmaQueuePair> q);
	void ScheduleDecreaseRateTfd(Ptr<RdmaQueuePair> q, uint32_t delta);

	void RateIncEventTimerTfd(Ptr<RdmaQueuePair> q);
