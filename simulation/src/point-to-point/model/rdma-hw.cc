...
TypeId RdmaHw::GetTypeId (void)
{
	static TypeId tid = TypeId ("ns3::RdmaHw")
		...
		.AddAttribute("RateHAI",
				...
				MakeDataRateChecker())
		.AddAttribute("tfdRatio",
				"Ratio for test",
				DoubleValue(1.0),
				MakeDoubleAccessor(&RdmaHw::tfd_ratio),
				MakeDoubleChecker<double>())
		.AddAttribute("tfdRateDecreaseInterval",
				"RateDecreaseInterval for test",
				DoubleValue(4.0),
				MakeDoubleAccessor(&RdmaHw::tfd_rateDecreaseInterval),
				MakeDoubleChecker<double>())
		.AddAttribute("tfdAlphaResumInterval",
				"The interval of resuming alpha for test",
				DoubleValue(55.0),
				MakeDoubleAccessor(&RdmaHw::tfd_alpha_resume_interval),
				MakeDoubleChecker<double>())
		.AddAttribute("tfdRPTimer",
				"The rate increase timer at RP in microseconds for test",
				DoubleValue(1500.0),
				MakeDoubleAccessor(&RdmaHw::tfd_rpgTimeReset),
				MakeDoubleChecker<double>())
		.AddAttribute("VarWin",
...
void RdmaHw::Setup(QpCompleteCallback cb){
	for (uint32_t i = 0; i < m_nic.size(); i++){
		...
		dev->m_gencnp = MakeCallback(&RdmaHw::GenerateMyCnp, this);
		dev->m_ccmode= MakeCallback(&RdmaHw::GetccMode, this);
		// config NIC
...
int RdmaHw::ReceiveCnp(Ptr<Packet> p, CustomHeader &ch){
	...
	std::string key = std::to_string(ch.ipid) + "_" + std::to_string(ch.sip);
	auto it=m_flowTable.find(key);
	if(it!=m_flowTable.end()){
		if(it->second<64-ch.m_ttl){
			it->second=64-ch.m_ttl;
		}
	}
	else m_flowTable[key]=64-ch.m_ttl;
	uint16_t maxHop=m_flowTable[key];
	tfd_ratio=(1.0*(64-ch.m_ttl))/maxHop; 
	// get qp
...
Ptr<Packet> RdmaHw::GenerateMyCnp(CustomHeader &ch){
	Ptr<RdmaRxQueuePair> rxQp = GetRxQp(ch.dip, ch.sip, ch.udp.dport, ch.udp.sport, ch.udp.pg, true);
	qbbHeader seqh;
   	Ptr<Packet> mycnp=Create<Packet>(std::max(60-14-20-(int)seqh.GetSerializedSize(), 0));
    mycnp->AddHeader(seqh);
    // add ipv4 header
    Ipv4Header head;	// Prepare IPv4 header
	head.SetDestination(Ipv4Address(ch.sip));
	head.SetSource(Ipv4Address(ch.dip));
    ...
    head.SetTtl(64);
	head.SetPayloadSize(mycnp->GetSize());
	head.SetIdentification(rxQp->m_ipid++);
	mycnp->AddHeader(head);
	// add ppp header
    PppHeader ppp;
	ppp.SetProtocol (0x0021);// EtherToPpp(0x800)
	mycnp->AddHeader (ppp);
	// return
	return mycnp;
}

uint32_t RdmaHw::GetccMode(){
	return m_cc_mode;
}
...
void RdmaHw::UpdateAlphaTfd(Ptr<RdmaQueuePair> q){
	#if PRINT_LOG
	//std::cout << Simulator::Now() << " alpha update:" << m_node->GetId() << ' ' << q->mlx.m_alpha << ' ' << (int)q->mlx.m_alpha_cnp_arrived << '\n';
	//printf("%lu alpha update: %08x %08x %u %u %.6lf->", Simulator::Now().GetTimeStep(), q->sip.Get(), q->dip.Get(), q->sport, q->dport, q->mlx.m_alpha);
	#endif
	...
	#endif
	q->mlx.m_alpha_cnp_arrived = false; // clear the CNP_arrived bit
	ScheduleUpdateAlphaTfd(q);
}
void RdmaHw::ScheduleUpdateAlphaTfd(Ptr<RdmaQueuePair> q){
    if(tfd_alpha_resume_interval!=(tfd_ratio)*m_alpha_resume_interval){
        Simulator::Cancel(q->mlx.m_eventUpdateAlpha);
        tfd_alpha_resume_interval=(tfd_ratio)*m_alpha_resume_interval;
}
	q->mlx.m_eventUpdateAlpha = Simulator::Schedule(NanoSeconds(tfd_alpha_resume_interval*1000), &RdmaHw::UpdateAlphaTfd, this, q);
}

void RdmaHw::cnp_received_tfd(Ptr<RdmaQueuePair> q){
		q->mlx.m_alpha_cnp_arrived = true; // set CNP_arrived bit for alpha update
		q->mlx.m_decrease_cnp_arrived = true; // set CNP_arrived bit for rate decrease
	if (q->mlx.m_first_cnp){
		// init alpha
		...
		ScheduleUpdateAlphaTfd(q);
		// schedule rate decrease
		ScheduleDecreaseRateTfd(q, 1); // add 1 ns to make sure rate decrease is after alpha update
		// set rate on first CNP
		q->mlx.m_targetRate = q->m_rate = m_rateOnFirstCNP * q->m_rate;
		q->mlx.m_first_cnp = false;
	}
		...
		if(tfd_alpha_resume_interval!=(tfd_ratio)*m_alpha_resume_interval){
			UpdateAlphaTfd(q);
			CheckRateDecrease_Tfd(q);
			ScheduleDecreaseRateTfd(q, 1);
		}
	}
}

void RdmaHw::CheckRateDecrease_Tfd(Ptr<RdmaQueuePair> q){
	if (q->mlx.m_decrease_cnp_arrived){
		#if PRINT_LOG
		printf("%lu rate dec: %08x %08x %u %u (%0.3lf %.3lf)->", Simulator::Now().GetTimeStep(), q->sip.Get(), q->dip.Get(), q->sport, q->dport, q->mlx.m_targetRate.GetBitRate() * 1e-9, q->m_rate.GetBitRate() * 1e-9);
		#endif
		...
		q->mlx.m_decrease_cnp_arrived = false;
		Simulator::Cancel(q->mlx.m_rpTimer);
        if(tfd_rpgTimeReset!=(tfd_ratio)*m_rpgTimeReset){
        tfd_rpgTimeReset=(tfd_ratio)*m_rpgTimeReset;
}
		q->mlx.m_rpTimer = Simulator::Schedule(NanoSeconds(m_rpgTimeReset*1000), &RdmaHw::RateIncEventTimerTfd, this, q);
		#if PRINT_LOG
		printf("(%.3lf %.3lf)\n", q->mlx.m_targetRate.GetBitRate() * 1e-9, q->m_rate.GetBitRate() * 1e-9);
		#endif
	}
...
void RdmaHw::CheckRateDecreaseTfd(Ptr<RdmaQueuePair> q){
	ScheduleDecreaseRateTfd(q, 0);
	CheckRateDecrease_Tfd(q);
}

void RdmaHw::ScheduleDecreaseRateTfd(Ptr<RdmaQueuePair> q, uint32_t delta){
    if(tfd_rateDecreaseInterval!=(tfd_ratio)*m_rateDecreaseInterval){
        Simulator::Cancel(q->mlx.m_eventDecreaseRate);
		tfd_rateDecreaseInterval=(tfd_ratio)*m_rateDecreaseInterval;

}
	q->mlx.m_eventDecreaseRate = Simulator::Schedule(NanoSeconds(tfd_rateDecreaseInterval*1000) + NanoSeconds(delta), &RdmaHw::CheckRateDecreaseTfd, this, q);
}

void RdmaHw::RateIncEventTimerTfd(Ptr<RdmaQueuePair> q){
    if(tfd_rpgTimeReset!=(tfd_ratio)*m_rpgTimeReset){
        tfd_rpgTimeReset=(tfd_ratio)*m_rpgTimeReset;
}
	q->mlx.m_rpTimer = Simulator::Schedule(NanoSeconds(m_rpgTimeReset*1000), &RdmaHw::RateIncEventTimerTfd, this, q);
