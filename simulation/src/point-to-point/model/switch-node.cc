...
bool SwitchNode::SwitchReceiveFromDevice(Ptr<NetDevice> device, Ptr<Packet> packet, CustomHeader &ch){
	std::string key = std::to_string(ch.sip) + "_" + std::to_string(ch.dip);
	/*auto it=m_flowTable.find(key);
	if(it!=m_flowTable.end()){
			it->second+=1;
	}
	else m_flowTable[key]=1;*/
	SendToDev(packet, ch);
	return true;
}
	...
	if (qIndex != 0){
		...
		if (m_ecnEnabled){
			...
			if(m_ccMode!=11)
				...
				egressCongested=m_mmu->ShouldsendCNBySize(ifIndex, qIndex);
			if (egressCongested){
				std::string key = std::to_string(ch.sip) + "_" + std::to_string(ch.dip);
				PppHeader ppp;
				Ipv4Header h;
...
int SwitchNode::GetCcMode(){
	return m_ccMode;
}
/*
int SwitchNode::getMinForTable(){
	int minVal = m_flowTable.begin()->second;
	 for (const auto& entry : m_flowTable) {
        ...
        if (entry.second!=0&&entry.second < minVal) {
            minVal = entry.second;
        }
	 }
	return minVal;
...
int SwitchNode::getMaxForTable(){
	int maxVal = m_flowTable.begin()->second;
	for (const auto& entry : m_flowTable)
