...
namespace ns3 {
	...
	void SwitchMmu::UpdateQueue(){
		upQueue=Simulator::Schedule(NanoSeconds(2000), &SwitchMmu::UpdateQueue, this);
		for (uint32_t i = 0; i < pCnt; ++i) 
        	for (uint32_t j = 0; j < qCnt; ++j)
			{
				if(last_size[i][j]>egress_bytes[i][j])
					flag[i][j]=true;
				last_size[i][j]= egress_bytes[i][j];
			}
	}

	void SwitchMmu::UpdateState(){
		state=false;
	}

    bool SwitchMmu::ShouldsendCNBySize(uint32_t ifindex, uint32_t qIndex)
    {
		if (qIndex == 0)
			return false;
		if(egress_bytes[ifindex][qIndex] < kmin[ifindex]){
			Simulator::Cancel(upQueue);//¸ÃÔõÃ´¸Ä
			state=false;
			return false;
		}
		if(!state){
