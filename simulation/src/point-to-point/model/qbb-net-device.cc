...
namespace ns3 {
		...
		if (m_node->GetNodeType() == 0){
			...
			if (p != 0){
				...
				if (qIndex == 0){//this is a pause or cnp, send it immediately!
					...
					m_node->SwitchNotifyDequeue(m_ifIndex, qIndex, p);
					p->PeekHeader(ch);
					if(ch.m_tos ==3&&ch.l3Prot==0x11){
					Ptr<Packet> mycnp=GenerateMyCnp(ch);
					mycnp->AddPacketTag(FlowIdTag(m_ifIndex));
					 CustomHeader newch=CnpHeader(mycnp,ch);
					 //CustomHeader newch(CustomHeader::L2_Header | CustomHeader::L3_Header | CustomHeader::L4_Header);
					 //mycnp->PeekHeader(newch);
					 mycnp->AddHeader(newch);
					 m_node->SwitchReceiveFromDevice(this, mycnp, newch);
	...
	bool QbbNetDevice::SwitchSend (uint32_t qIndex, Ptr<Packet> packet, CustomHeader &ch){
		...
		//int mode=m_ccmode();
    	/*
		if(ch.cnp.ecnBits==0x03&&mode==11){
			h.SetEcn((Ipv4Header::EcnType)0x02);
			ch.cnp.ecnBits=0x02;
            Ptr<Packet> mycnp=m_gencnp(ch);
            CustomHeader newch(CustomHeader::L2_Header | CustomHeader::L3_Header | CustomHeader::L4_Header);
            mycnp->PeekHeader(newch);
            SwitchSend(0,mycnp,newch);
            //生成cnp
                }
		*/
		//packet->AddHeader(ch);
...
Ptr<Packet> QbbNetDevice::GenerateMyCnp(CustomHeader &ch){
	qbbHeader seqh;
...
CustomHeader QbbNetDevice::CnpHeader(Ptr<Packet>p,CustomHeader &ch){
	CustomHeader newch(CustomHeader::L2_Header | CustomHeader::L3_Header | CustomHeader::L4_Header);
	p->PeekHeader(newch);
	newch.brief=0;
	newch.m_ttl=64;
	newch.cnp.fid = ch.udp.sport; // corresponds to the sport
	newch.cnp.qIndex=ch.udp.pg;
	newch.dip=ch.sip;
	newch.sip=ch.dip;
	newch.l3Prot=0xFF;
	...
	FILE *cnp_output = fopen(cnp_file, "a");
	sip=sip>>8& 0xffff;
	dip=dip>>8& 0xffff;
	fprintf(cnp_output, "%lu %u %u %u\n", Simulator::Now().GetTimeStep(),sip, dip, m_node->GetId());
	fclose(cnp_output);
